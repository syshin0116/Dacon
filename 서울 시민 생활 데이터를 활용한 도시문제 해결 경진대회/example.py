...
geo.head()
df_geo = geo.iloc[:,[2,9]]
## pd.merge(left_table, right_table, on = 기준 컬럼)
rdata = pd.merge(data, df_geo, on = 'adm_cd')
rdata.head()
data.head()
type(rdata)
## rdata를 geodataframe 형태로 바꿔줌
## polygon
...
# 실습
data_tmp2 = data_merge[data_merge['연령대'] == 20][data_merge['성별'] == 2]
idx = (data_merge['연령대'] == 20) & (data_merge['성별'] == 2)
data_tmp2.loc[idx,:]
data_tmp2.head()
type(data_tmp2)
# 시각화,  column 에 시각화할 필드명을 입력
fig = plt.figure()
data_tmp2.plot(column='평일 외출이 적은 집단',
                 legend=True,
                 cmap='YlGn',
                 edgecolor='k',
                 legend_kwds={'label': '명'})
